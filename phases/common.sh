...
debug() {
  echo "[$(date '+%H:%M:%S')] [DEBUG] $*" >> "${LOG%/*}/debug.log"
}
debug_tool_start() {
  local tool="$1"; shift
  echo "[$(date '+%H:%M:%S')] [DEBUG] >>>>>>>>>> STARTING: $tool <<<<<<<<<<" >> "${LOG%/*}/debug.log"
  echo "[$(date '+%H:%M:%S')] [DEBUG]   Command: $*" >> "${LOG%/*}/debug.log"
  echo "[$(date '+%H:%M:%S')] [DEBUG]   Working dir: $(pwd)" >> "${LOG%/*}/debug.log"
  local base_tool="${tool%%/*}"
  local tool_path
  tool_path=$(command -v "$base_tool" 2>/dev/null || echo 'NOT FOUND')
  echo "[$(date '+%H:%M:%S')] [DEBUG]   Tool binary: $tool_path" >> "${LOG%/*}/debug.log"
  if [[ "$tool_path" != "NOT FOUND" && -f "$tool_path" ]]; then
    local tool_version
    tool_version=$("$tool_path" --version 2>/dev/null | head -1 || echo "unknown")
    echo "[$(date '+%H:%M:%S')] [DEBUG]   Tool version: $tool_version" >> "${LOG%/*}/debug.log"
  fi
}
debug_tool_end() {
  local tool="$1" exit_code="$2" output_file="${3:-}" duration="${4:-}"
  echo "[$(date '+%H:%M:%S')] [DEBUG] <<<<<<<<<< FINISHED: $tool (exit=$exit_code, time=${duration}s) >>>>>>>>>>" >> "${LOG%/*}/debug.log"
  if [[ -n "$output_file" && -f "$output_file" ]]; then
    local lines size
    lines=$(wc -l < "$output_file" 2>/dev/null | tr -d ' ')
    size=$(wc -c < "$output_file" 2>/dev/null | tr -d ' ')
    echo "[$(date '+%H:%M:%S')] [DEBUG]   Output: $output_file ($lines lines, $size bytes)" >> "${LOG%/*}/debug.log"
    if [[ "$lines" -gt 0 ]] 2>/dev/null; then
      echo "[$(date '+%H:%M:%S')] [DEBUG]   First 3 lines:" >> "${LOG%/*}/debug.log"
      head -3 "$output_file" 2>/dev/null | while IFS= read -r _dbg_line; do
        echo "[$(date '+%H:%M:%S')] [DEBUG]     $_dbg_line" >> "${LOG%/*}/debug.log"
      done
    fi
  elif [[ -n "$output_file" ]]; then
    echo "[$(date '+%H:%M:%S')] [DEBUG]   Output file NOT FOUND: $output_file" >> "${LOG%/*}/debug.log"
  fi
}
...
scan_url() {
  ...
  echo "[$(date '+%H:%M:%S')] [DEBUG] >>>>>>>>>> STARTING: dalfox <<<<<<<<<<<" >> "$_O/logs/debug.log"
  echo "[$(date '+%H:%M:%S')] [DEBUG]   Target URL: $url" >> "$_O/logs/debug.log"
  {
    flock 9
  ...
  echo "[$(date '+%H:%M:%S')] [DEBUG]   dalfox args: ${dalfox_args[*]}" >> "$_O/logs/debug.log"
  local dalfox_exit=0
  ...
  echo "[$(date '+%H:%M:%S')] [DEBUG] <<<<<<<<<< FINISHED: dalfox (exit=$dalfox_exit, time=${_df_dur}s) >>>>>>>>>>" >> "$_O/logs/debug.log"
  echo "[$(date '+%H:%M:%S')] [DEBUG]   Result length: ${#result} chars" >> "$_O/logs/debug.log"
  ...
  if printf '%s\n' "$result" | grep -qiE '\[POC\].*\[[GRV]\]|Found.*XSS|\[V\]\s|XSS\s+Found|Reflected.*XSS'; then
    echo "[$(date '+%H:%M:%S')] [FOUND]   *** XSS DETECTED for $url ***" >> "$_O/logs/reflexionx.log"
...
*Payload info details*: \`poc/${hash}.txt\`" \
    ...
    echo "[$(date '+%H:%M:%S')] [DEBUG]   No XSS found for $url" >> "$_O/logs/debug.log"
  fi
  {
    flock 9
...
scan_xsstrike() {
  ...
  echo "[$(date '+%H:%M:%S')] [DEBUG] >>>>>>>>>> STARTING: xsstrike <<<<<<<<<<<" >> "$_O/logs/debug.log"
  echo "[$(date '+%H:%M:%S')] [DEBUG]   Target URL: $url" >> "$_O/logs/debug.log"
  ...
  echo "[$(date '+%H:%M:%S')] [DEBUG] <<<<<<<<<< FINISHED: xsstrike (time=${_xs_dur}s) >>>>>>>>>>" >> "$_O/logs/debug.log"
  { flock 9; printf '%s\n\n' "$result" >> "$_O/xsstrike_results.txt"; } 9>"$_S/xsstrike_w.lock"
  if printf '%s\n' "$result" | grep -qiE '\[\+\].*[Xx][Ss][Ss]|payload:|vulnerable'; then
    echo "[$(date '+%H:%M:%S')] [FOUND]   *** XSStrike HIT for $url ***" >> "$_O/logs/reflexionx.log"
    local hash; hash=$(printf '%s' "$url" | md5sum | cut -d' ' -f1)
    ...
    echo "[$(date '+%H:%M:%S')] [DEBUG]   No XSStrike hit for $url" >> "$_O/logs/debug.log"
  fi
...
dashboard() {
  ...
  while [[ -f "${STATE:-/tmp/reflexionx_state}/running" ]]; do
    ...
    [[ "$found" =~ ^[0-9]+$ ]] || found=0
    [[ "$p_done" =~ ^[0-9]+$ ]] || p_done=0
    [[ "$p_total" =~ ^[0-9]+$ ]] || p_total=0
    [[ "$p_start" =~ ^[0-9]+$ ]] || p_start=0
...
set_phase() {
  ...
  if [[ ! "$total_val" =~ ^[0-9]+$ ]]; then
    if [[ -f "$total_val" ]]; then
      total_val=$(count_lines "$total_val")
    elif [[ -f "$OUT/$total_val" ]]; then
      total_val=$(count_lines "$OUT/$total_val")
    else
      total_val=0
    fi
  fi
  ...
  state_set "phase_total" "$total_val"
  state_set "phase_start" "$(date +%s)"
  log "Phase: $1  (count: $total_val)"
}
count_lines() {
  [[ ! -f "$1" ]] && { echo 0; return; }
  local _c
