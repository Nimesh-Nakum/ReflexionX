...
def extract_forms(html, base_url=""):
    ...
    for form_match in form_pattern.finditer(html):
        ...
        attrs = {}
        for am in attr_pattern.finditer(form_tag[:form_match.end() - len(form_match.group(1)) - 7]):
            key = am.group(1).lower()
            val = am.group(2) or am.group(3) or ""
            attrs[key] = val
...
if __name__ == "__main__":
    ...
    if args.urls and os.path.isfile(args.urls):
        input_urls = [l.strip() for l in open(args.urls) if l.strip() and l.startswith("http")]
        ...
        input_urls = [l.strip() for l in open(args.live) if l.strip() and l.startswith("http")]
    ...
    if not targets:
        print("[!] No forms found in any fetched pages", file=sys.stderr)
        sys.exit(1)
