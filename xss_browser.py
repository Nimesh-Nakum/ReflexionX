...
def check_execution(page, timeout_ms=3000):
    """Check if XSS payload executed by reading the window flag."""
    try:
...
def simulate_events(page):
    """Simulate user events that might trigger XSS execution.
    v2.0.0: expanded with hover, change, keydown, scroll, touch, iframe.
    """
    # ── Click events ─────────────────────────────────────────
    try:
        for sel in ["input", "button", "a", "textarea", "select", "details",
                     "[onclick]", "[ontoggle]"]:
    ...
    # ── Focus events ─────────────────────────────────────────
    try:
        for sel in ["input", "textarea", "[tabindex]", "[onfocus]", "[autofocus]"]:
            elements = page.query_selector_all(sel)
            for el in elements[:3]:
                try:
                    if el.is_visible():
    ...
    try:
        for sel in ["[onmouseover]", "[onmouseenter]", "a", "button", "div[class]"]:
            elements = page.query_selector_all(sel)
            for el in elements[:3]:
                try:
                    if el.is_visible():
    ...
    try:
        for sel in ["[onkeydown]", "[onkeyup]", "[onkeypress]", "input", "textarea"]:
            elements = page.query_selector_all(sel)
            for el in elements[:2]:
                try:
                    if el.is_visible():
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # If --cookie was provided, first visit the target's login page
        # and set the cookie, then test XSS pages with that session.
        # Without this, authenticated targets return login pages and
        # check_execution() always returns False.
        if extra_headers and "Cookie" in extra_headers:
            # Cookies are already set via extra_http_headers in browser.new_context.
            # No need to visit login.php — just proceed with authenticated session.
            print("  [*] Using provided cookie for authenticated session")

        # Handle dialogs
        dialog_detected = {"value": False}
        def on_dialog(dialog):
            if CANARY in dialog.message:
                ...
                for param_info in url_ctx.get("parameters", [])[:2]:
                    ...
                    for attempt in range(1, retry_engine.max_retries + 1):
                        ...
                        alternatives = retry_engine.suggest(
                            ...
                            attempt=attempt)
                        for alt_payload, strategy in alternatives[:3]:
                            if rate_ctrl:
                                rate_ctrl.wait()
                            try:
                                page.evaluate("() => { window._xss_confirmed = null; }")
        ...
        if HAS_FRAGMENT and getattr(args, 'fragment_urls', None) and os.path.isfile(args.fragment_urls):
            ...
            for frag_url, frag_payload, _strategy in frag_entries[:30]:
                try:
                    page.evaluate("() => { window._xss_confirmed = null; }")
